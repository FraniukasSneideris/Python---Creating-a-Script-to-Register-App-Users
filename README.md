...
You can run the system from the command line to validate or register users individually, or to process bulk user registrations from a CSV file. If you wish to try the bulk registration, you can try it with the files I've added to the repositor: *user_data.csv* (clean data), or *tampered_user_data_(...).csv* (user_data.csv with alterations in name, email and password respectively).
